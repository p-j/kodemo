...
export const picturesRouter = router({
  ...
  findFirstPictureOrThrow: publicProcedure.input(PictureFindFirstSchema).query(async ({ ctx, input }) => {
    const findFirstPictureOrThrow = await ctx.prisma.picture.findFirstOrThrow(input)
    return findFirstPictureOrThrow
  }),
  ...
  findUniquePictureOrThrow: publicProcedure.input(PictureFindUniqueSchema).query(async ({ ctx, input }) => {
